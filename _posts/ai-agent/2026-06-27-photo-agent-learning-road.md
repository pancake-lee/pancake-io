...
最初的路线是**纯 Python 单栈**（FastAPI + LangChain + Chroma）。但 Day 1 就发现工作量被严重低估——FastAPI 路由、LangChain 编排、向量库管理、错误处理……5 天周期根本写不完。

于是第一次转向：**用 Dify 替代 Python AI 服务层。**
...
纯 Python 单栈
  → Go + Python 双栈（复用 Go 工程经验）
    → Dify + Go（降低 Python 工作量）
      → Dify + Go + Python 三栈（重新引入 Python 做精细路由）
```
...
### 火山引擎 Embedding URL 的 Dify 兼容问题
...
**并发安全**花了一点心思：中间保存时先 `Mu.Lock()` 深拷贝 map → 解锁 → Marshal + 原子写入（临时文件 + `os.Rename`）。否则 `json.MarshalIndent` 遍历 map 时，其他 goroutine 写入会触发 `fatal error: concurrent map read and map write`。
...
3. **从"技术选型追求最优"到"选择当下最合适的"**。四次架构转向不是因为"之前的选错了"，而是因为需求在演进。MVP 阶段的过度设计比技术选型失误更致命。

---

> **项目地址**：[github.com/pancake-lee/photo-agent](https://github.com/pancake-lee/photo-agent)（MIT 开源）
>
> **相关阅读**：[PFlow 设计哲学：不是又一个 Agent，而是一张 AI 会话的沙盘指挥图](/pancake-io/posts/pflow-design-philosophy/) · [DDIA 笔记系列](/pancake-io/categories/backend/)
